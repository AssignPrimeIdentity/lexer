...
This scheme is generated by developing an application that involving at least 6 x 19 = 114 repositories all together in the same time. So although the code is already running but unfortunately it is not yet user friendly as it could run only in _[GitHub API Platform](https://docs.github.com/en/developers)_ as the repos providers.
...
Let's call it as **eQ19 Quantum** which could simply form as below:

```
6 + 6 x (6+(6+6)) = 6+(6x6) + 6x(6+6) = 6 x 19 = π(6 & 19) = eQ19
```

On that case we consider _This Statement_ need to be announced. Out of the formation, you are welcome to use whatever the items in this project.
